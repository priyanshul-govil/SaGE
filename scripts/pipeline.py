...
def model_output(df, model_name):
    if model_name in ["gpt-3.5-turbo", "gpt-4"]:
        df['model_output'] = df['paraphrased'].progress_apply(
            lambda q: get_gpt_response(q, model_name))
    else:
...
def read_data(dataset):
    if dataset == 'mcc':
        return pd.read_csv('data/all/q5k.csv')
    elif dataset == 'gsm':
        with open('data/datasets/GSM8k.jsonl', 'r') as file:
            data = [json.loads(line) for line in file]
        df = pd.DataFrame(data)
        df['question_id'] = range(1, len(df) + 1)
        return df[['question_id', 'question']]
    elif dataset == 'truthfulqa':
        df = pd.read_csv('data/datasets/TruthfulQA.csv')
        df['question_id'] = range(1, len(df) + 1)
        return df[['question_id', 'Question']].rename(columns={'Question': 'question'})
    else:
        raise ValueError("Invalid dataset option. Choose from ['mcc', 'gsm', 'truthfulqa'].")

def main(args):
    dataset = args.dataset
    if not dataset:
        raise ValueError("Dataset argument is required. Choose from ['mcc', 'gsm', 'truthfulqa'].")

    input_df = read_data(dataset)
