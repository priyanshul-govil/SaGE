...
"""
    ...
    for index, row in tqdm(df.iterrows(), "para", total=len(df)):
        ...
        for paraphrase_data in valid_paraphrases:
            paraphrased_data.append({"question_id": row["question_id"], "question": question, "action1": row["action1"], "action2": row["action2"], **paraphrase_data})
    return pd.DataFrame(paraphrased_data)

def main(args):
    dataset = "data/datasets/moralchoice_all.csv"
